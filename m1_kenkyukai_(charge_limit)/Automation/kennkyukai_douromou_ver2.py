...
count = 0
L = 10 #SAの大きさ
d_limit = 3.5 #山間部避難所は radius + d_limt より離れた場所に生成 (30,1.0のとき 2.5)
point_num = 51 #全部で生成する点の数
center_point = (5, 5)  # 中央の点
circle_radius = 3  # 円の半径
num_points_outer = 10  # 円外の点の数
...
radii = np.random.uniform(0+0.3, circle_radius-0.2, num_points_inner - 1)
...
radii_outer = np.random.uniform(d_limit, L/2-0.1, num_points_outer)#circle_radiusから10/2だけの距離の間に
...
plt.triplot(points_inner[:, 0], points_inner[:, 1], tri_inner.simplices, color='k', linewidth=linewi)
...
for i in range(num_points_outer):
    ...
    plt.plot([points[outer_point_idx, 0], points[closest_inner_point_idx, 0]],
             [points[outer_point_idx, 1], points[closest_inner_point_idx, 1]], color='k', linewidth=linewi)#線の太さ
