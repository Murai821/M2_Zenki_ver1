#r=0.5~4.5まで変更し、さらにそれぞれに対してシード値を変化させて複数回実行するプログラム「run_0.5to4.5_sankanbu.sh」
...
circle_radius = int(sys.argv[3])  # 円の半径
...
seed = int(sys.argv[1])
np.random.seed(seed)
print("pythonfile : seed = " + str(seed))
# np.random.seed(45)
...
#radii = np.random.uniform(0+0.5, circle_radius, num_points_inner - 1) # r >= 1.0 のとき
radii = np.random.uniform(0, circle_radius, num_points_inner - 1) # r=0.5のとき
...
radii_outer = np.random.uniform(d_limit, L/2, num_points_outer)#circle_radiusから10/2だけの距離の間に
