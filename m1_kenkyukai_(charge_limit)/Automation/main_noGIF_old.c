...
int main(void)
{
    ...
    int re_load_num = 100;         // 配送センターで一度に積載する物資の数
    int re_finish_num = 100;       // シミュレーション終了物資量
    ...
    for (i = 0; i < M; i++)
    {
        v[i].re = 100; // それぞれの避難所に10個ずつ物資を届ける
    }
        ...
        for (i = 0; i < M; i++)
        {
            ...
            { // ③それ以外において、targetに到達したらcurrentとtarget更新
                ...
                if (current[i] == ind_relief[i] && ind_relief[i] <= (i + 1) * 10)
                {
                    ...
                    v[i].re -= 10;                 // 配送車の物資減少
                    new_p[ind_relief[i]].re += 10; // 避難所の物資増加
                    ind_relief[i] += 1;            // index更新
            ...
            { // 自由飛行モードならtargetの配送車に向かって飛行
                ...
                if ((d_n_cos[i] < 0 && drone[i].y < drone[i].yt) || (d_n_cos[i] > 0 && drone[i].y > drone[i].yt))
                {
                    ...
                    */
                    ...
                    if (v[drone[i].target_num].charge_amount >= MAX_TVchargeable) // TVでの充電量が規定量を超えた場合はそのドローンは充電できずに待機
                    {
                        drone[i].free_mode = FALSE;
