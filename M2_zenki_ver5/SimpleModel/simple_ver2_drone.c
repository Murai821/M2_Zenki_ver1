 ...
 * 必要環境: gcc, gnuplot
 *
 * 7/8：複数ドローンを周回させる処理追加、ドローンによる情報収集は未実装
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
// === 情報発生システム（ポアソン過程） ===
#define LAMBDA 0.2    // ポアソン到着率 [件/時間] | 1時間に平均0.5件の情報発生
 ...
 * @param drone_x ドローンのX座標配列[ドローンID]
 * @param drone_y ドローンのY座標配列[ドローンID]
 * @param drone_active ドローンの活動状態配列[ドローンID]（0:未出発, 1:活動中）
 * @param elapsed_time 経過時間（秒）
 * @param info_list 情報リスト配列
 * @param info_count 発生済み情報の総数
 */
void plot_frame(FILE *pipe, double stop_coords[][2], double vehicle_x, double vehicle_y, double drone_x[], double drone_y[], int drone_active[], double elapsed_time, Info *info_list, int info_count)
{
    ...
    if (ND > 0)
    {
        ...
        for (int i = 0; i < ND; i++)
        {
            if (drone_active[i]) // アクティブ状態のドローンのみ描画
            {
                fprintf(pipe, "%.1f %.1f\n", drone_x[i], drone_y[i]);
                active_drone_count++;
            }
...
int main(void)
{
    ...
    // === ドローン制御変数（複数台対応） ===
    double drone_angle[ND];          // 各ドローンの現在角度（ラジアン）
    double drone_x[ND], drone_y[ND]; // 各ドローンの現在位置（メートル）
    int drone_active[ND];            // 各ドローンの活動状態（0:未出発, 1:活動中）
    double drone_start_time[ND];     // 各ドローンの出発時刻（秒）
    ...
    for (int i = 0; i < ND; i++)
    {
        drone_angle[i] = PI / 2.0;                          // 全て集積所（12時位置、π/2ラジアン）から開始
        drone_x[i] = R * cos(drone_angle[i]);               // 初期X座標
        drone_y[i] = R * sin(drone_angle[i]);               // 初期Y座標
        drone_active[i] = 0;                                // 初期状態：未出発
        drone_start_time[i] = i * drone_departure_interval; // 出発時刻（等間隔分散）
    }
    ...
    while (lap_count < NT)
    {
        ...
        {
            // 停止開始時の描画
            plot_frame(gnuplot_pipe, stop_coords, current_x, current_y, drone_x, drone_y, drone_active, elapsed_time, info_list, info_count);
            ...
            while (elapsed_time < stop_end_time)
            {
                ...
                for (int i = 0; i < ND; i++)
                {
                    ...
                    if (!drone_active[i] && elapsed_time >= drone_start_time[i])
                    {
                        drone_active[i] = 1;
                        printf("ドローン%d: 出発（時刻%.1f秒）\n", i + 1, elapsed_time);
                    }

                    // アクティブドローンの位置更新（反時計回り移動）
                    if (drone_active[i])
                    {
                        // 角速度計算: ω = v/r [ラジアン/秒]
                        double drone_angle_increment = (V_DRONE / R) * time_step;
                        drone_angle[i] += drone_angle_increment; // 反時計回り（角度増加）

                        // 角度正規化（2π超えの場合は2π減算）
                        if (drone_angle[i] > 2.0 * PI)
                        {
                            drone_angle[i] -= 2.0 * PI;
                        }

                        // 直交座標への変換
                        drone_x[i] = R * cos(drone_angle[i]);
                        drone_y[i] = R * sin(drone_angle[i]);
                    }
                }
            }
    ...
    if (collected_count > 0)
    {
        ...
        printf("Tc総計: %.2f秒 (%.2f時間)\n", total_tc, total_tc / 3600.0);
    }
    else
    {
        printf("Tc平均値: 計算不可（回収情報なし）\n");
    }
