...
void update_drone_state(DroneInfo *drone, double stop_coords[][2], double elapsed_time, double time_step, Info *info_list, int info_count, ShelterSupply *shelter_supplies);
...
void plot_frame(FILE *pipe, double stop_coords[][2], double vehicle_x, double vehicle_y, DroneInfo drones[], double elapsed_time, Info *info_list, int info_count, ShelterSupply shelter_supplies[], SupplyVehicle *supply_vehicle)
{
    ...
    const double supply_offset = 200.0; // 避難所からの表示オフセット距離
...
int main(void)
{
            ...
            else
            {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    ...
                    if (!info_list[i].collected && info_list[i].shelter_id == current_stop_idx)
                    {
                        ...
                        // 車両に余剰物資Bの残量があり、配送が未完了の場合のみ実行
                        if (supply_vehicle.remaining_extra_supply_b > 0 && !info_list[i].delivery_completed)
                        {
                            // === 配送量の決定 ===
                            // min(需要量, 車両残量) の原則で配送量を決定
                            // 需要量が車両残量より多い場合は、車両残量分のみ配送
                            double delivery_amount = (info_list[i].extra_supply_demand > supply_vehicle.remaining_extra_supply_b) ? supply_vehicle.remaining_extra_supply_b : info_list[i].extra_supply_demand;
            ...
            while (elapsed_time < stop_end_time)
            {
                ...
                for (int i = 0; i < ND; i++)
                {
                    ...
                    if (drones[i].active)
                    {
                        ...
                        // ドローンの状態更新
                        update_drone_state(&drones[i], stop_coords, elapsed_time, time_step, info_list, info_count, shelter_supplies);
                    }
                }
            }
                ...
                if (drones[i].active)
                {
                    ...
                    if (drones[i].state == DRONE_PATROL)
                    {
                        ...
                        if (detected_shelter > 0)
                        {
                            ...
                            drones[i].state = DRONE_TO_DEPOT;            // 集積所への飛行状態に変更
    ...
    {
        // 現在は車両回収分のみTc統計を記録（total_tc, collected_count）
        // ドローン回収分は即座に回収されるため、別途実装が必要
        double average_tc = total_tc / collected_count; // 車両回収のTc平均値
