...
int find_vehicle_next_depot_ver2(SupplyVehicle supply_vehicle[], int current_stop_idx[], int next_stop_idx[], DroneInfo drones[], int i, double stop_coords[][2], int dis_idx[NDI], int ndi_count, double vehicle_x[], double vehicle_y[], double ve_stop_end_time, int ve_stop_flag, int is_depot, double elapsed_time);
            ...
            for (int v = 0; v < NV; v++)
            {
                ...
                if (is_depot) // current_stop_idx[v] == 0など集積所インデックスと一致したとき
                {
                    ...
                    for (int i = 0; i < info_count; i++)
                    {
                        ...
                        // 集積所 -> 運搬車両
                        else if (supply_vehicle[v].collect_info_id[i] == 0 && facilities[current_stop_idx[v]].collect_info_id[i] == 1)
                        {
                    ...
                    for (int i = 0; i < info_count; i++)
                    {
                        if (facilities[current_stop_idx[v]].collect_info_id[i] == 1 && supply_vehicle[v].demand_supply_loaded_flag[i] == 0) // 集積所に物資運搬車両またはドローンによって共有された情報に基づき、要求の物資を積載
                        {
                ...
                while (elapsed_time < stop_end_time)
                {
                    ...
                    if (is_depot) // current_stop_idx[] == 0など集積所インデックスと一致したとき
                    {
                        ...
                        {
                            // 集積所 -> 運搬車両
                            if (supply_vehicle[v].collect_info_id[i] == 0 && facilities[current_stop_idx[v]].collect_info_id[i] == 1)
                            {
                                supply_vehicle[v].collect_info_id[i] = 1; // 運搬車両での情報収集フラグを設定
                        ...
                        for (int i = 0; i < info_count; i++)
                        {
                            if (facilities[current_stop_idx[v]].collect_info_id[i] == 1) // 集積所に物資運搬車両またはドローンによって共有された情報に基づき、要求の物資を積載
                            {
                                supply_vehicle[v].demand_supply_loaded_flag[i] = 1; // 要求物資積載フラグをセット
                            }
                        }
