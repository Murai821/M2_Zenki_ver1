...
// === 余剰物資A配送手法の選択 ===
// #define DELIVERY_METHOD_COORDINATE 1                // 手法2：車両がドローンの運搬状況を考慮：（避難所に到着し、ドローンが運搬中であるなら残りの余剰物資を避難所に届ける）
...
typedef struct
{
    ...
    int A_delivery_completed;          // 余剰物資A配送完了フラグ（0:未完了, 1:完了）
    int B_delivery_completed;          // 余剰物資B配送完了フラグ（0:未完了, 1:完了）
    double A_delivery_completion_time; // 余剰物資A配送完了時刻（秒）| -1の場合は未完了
    double B_delivery_completion_time; // 余剰物資Bの配送完了時刻（秒）| -1の場合は未完了
    int fast_detection_flag;           // 迅速発見フラグ（1:車両がドローンによって共有された情報を集積所で先に発見, 0:遅れて発見）
} Info;
...
void update_drone_state(DroneInfo *drone, DroneInfo drones[], int drone_count, double stop_coords[][2], double elapsed_time, double time_step, Info *info_list, int info_count, Facility *facilities, double *total_extra_supply_by_drone, int *drone_delivery_count, int drone_index, int dis_idx[NDI], SupplyVehicle supply_vehicle[], int current_stop_idx[], int next_stop_idx[], double vehicle_x[], double vehicle_y[])
{
    ...
    case DRONE_AT_DEPOT:
        ...
        if (elapsed_time >= drone->state_start_time + T_DRONE_STOP)
        {
            ...
            if (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE)
            {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    if (info_list[i].shelter_id == drone->target_shelter && !info_list[i].A_delivery_completed)
                    {
    ...
    case DRONE_AT_SHELTER:
        ...
        if (elapsed_time >= drone->state_start_time + T_DRONE_STOP)
        {
            ...
            for (int i = 0; i < info_count; i++)
            {
                if (info_list[i].shelter_id == drone->target_shelter && !info_list[i].A_delivery_completed && ((DELIVERY_METHOD != DELIVERY_METHOD_MULTI_DRONE && drone->delivery_info_index == i) || (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE)) && drone->delivery_info_index == i) // A_delivery_completedフラグが立っていない場合(手法2のみ情報のインデックス参照)
                {
                    ...
                    printf("ドローン: 避難所%dに余剰物資A %.0fkg配送（累計 %.0f/%.0fkg）\n",
                           drone->target_shelter, drone->carrying_extra_supply,
                           info_list[i].A_extra_supply_delivered, info_list[i].A_extra_supply_demand);
                    ...
                    *total_extra_supply_by_drone += drone->carrying_extra_supply;
                    ...
                    if (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE)
                    {
                        ...
                        if (info_list[i].A_extra_supply_delivered + total_other_drone_transport >= info_list[i].A_extra_supply_demand) // 避難所に届けられた物資量と他のドローンの運搬中の物資量が需要量を超えた場合
                        {
                            ...
                            {
                                printf("ドローン: 避難所%dの余剰物資A配送完了（他のドローンは継続中）他のドローン積載量:%f, 避難所%d の Req:%f \n", drone->target_shelter, total_other_drone_transport, drone->target_shelter, info_list[i].A_extra_supply_demand - info_list[i].A_extra_supply_delivered);
                            }
                            else // 自分が最後にとどけた場合
                            {
                                info_list[i].A_delivery_completed = 1;
                                info_list[i].A_delivery_completion_time = elapsed_time;
                                printf("ドローン: 避難所%dの余剰物資A配送完了（全ドローン終了）, 避難所%d の Req:%f \n", drone->target_shelter, drone->target_shelter, info_list[i].A_extra_supply_demand - info_list[i].A_extra_supply_delivered);
                            }
...
double calculate_drone_transport_amount(DroneInfo drones[], int drone_count, int shelter_id, Info *info_list, int info_count, int info_index)
{
    ...
    for (int i = 0; i < info_count; i++)
    {
        if (info_list[i].shelter_id == shelter_id && !info_list[i].A_delivery_completed && info_index == i)
        {
 ...
 * 2. 配送が未完了（A_delivery_completed = 0）
...
int should_drone_join_transport(DroneInfo *drone, DroneInfo drones[], int drone_count, int shelter_id, Info *info_list, int info_count)
{
    ...
    {
        // 指定避難所IDかつ未配送完了
        if (info_list[i].shelter_id == shelter_id && info_list[i].A_delivery_completed == 0)
        {
...
double calculate_all_drones_transport_amount(DroneInfo *drone, DroneInfo drones[], int drone_count, int shelter_id, Info *info_list, int info_count)
{
    ...
    for (int i = 0; i < info_count; i++)
    {
        if (info_list[i].shelter_id == shelter_id && !info_list[i].A_delivery_completed)
        {
            // この避難所に対して運搬中のドローンを検索
            for (int j = 0; j < drone_count; j++)
            {
...
void plot_frame(FILE *pipe, double stop_coords[][2], double vehicle_x[], double vehicle_y[], DroneInfo drones[], double elapsed_time, Info *info_list, int info_count, Facility facilities[], SupplyVehicle supply_vehicle[], int dis_idx[NDI])
{
    ...
    for (int i = 1; i <= NS + NDI - 1; i++) // 避難所のみ（集積所は除く）
    {
        ...
        {
            if (info_list[j].shelter_id == i && !info_list[j].A_delivery_completed &&
...
int main(int argc, char *argv[])
{
    ...
    supply_vehicle[0].remaining_extra_supply_b = EXTRA_SUPPLY_B; // 余剰物資A: 1000kg
    ...
    while (lap_count < NT)
    {
        ...
        while (elapsed_time >= next_info_time && info_count < MAX_INFO)
        {
            ...
            info_list[info_count].A_extra_supply_delivered = 0.0;                         // 配送済み量: 初期0kg
            info_list[info_count].B_extra_supply_delivered = 0.0;                         // B配送済み量: 初期0kg
            info_list[info_count].A_delivery_completed = 0;                               // 配送完了フラグ: 未完了
            info_list[info_count].B_delivery_completed = 0;                               // 余剰物資B配送完了フラグ: 未完了
            info_list[info_count].A_delivery_completion_time = -1;                        // 配送完了時刻: 未完了
            ...
            else /***************** 避難所に停止する場合******************************************/
            {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    ...
                    if (!info_list[i].collected && info_list[i].shelter_id == current_stop_idx[0] && supply_vehicle[0].remaining_extra_supply_b > 0)
                    {
                        ...
                        // 車両に余剰物資の残量があり、配送が未完了の場合のみ実行（手法2においてどちらも実行される）
                        if (supply_vehicle[0].remaining_extra_supply_b > 0 && !info_list[i].A_delivery_completed && supply_vehicle[0].demand_supply_loaded_flag[i] == 1)
                        {
                            ...
                            if (delivery_amount > 0)
                            {
                                ...
                                else
                                {
                                    printf("  余剰物資A配送: %.0fkg配送（残り需要 %.0fkg）\n",
                                           delivery_amount, info_list[i].A_extra_supply_demand - info_list[i].A_extra_supply_delivered);
                                }
                            }
                            else
                            {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    if (ND >= 0) // ドローンを導入しない場合は余剰物資Aも物資運搬車両が全て配送する、また、ドローンを導入する場合もドローンで運搬しきれない物資Aも、集積所で所望の物資を積載した上で運搬
                    {
                        ...
                        if (info_list[i].shelter_id == current_stop_idx[0] && supply_vehicle[0].demand_supply_loaded_flag[i] == 1 && supply_vehicle[0].remaining_extra_supply_b > 0 && !info_list[i].A_delivery_completed)
                        {
                            double delivery_amount = 0.0;
                            ...
                            else // 情報を検出しなかった場合
                            {
                                if (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE) // 手法3のとき
                                {
                                    ...
                                    if (cooperative_shelter > 0)
                                    {
                                        ...
                                        for (int j = 0; j < info_count; j++)
                                        {
                                            if (info_list[j].shelter_id == cooperative_shelter && !info_list[j].A_delivery_completed)
                                            {
    ...
    if (numerical_data_file != NULL)
    {
        ...
        for (int i = 0; i < info_count; i++)
        {
            if (info_list[i].A_delivery_completed && info_list[i].A_delivery_completion_time > 0)
            {
                double tr = info_list[i].A_delivery_completion_time - info_list[i].generation_time;
                total_tr_time += tr;
        ...
        else
        {
            fprintf(numerical_data_file, "%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n",
                    ...
                    total_extra_supply_by_vehicle / NT, total_extra_supply_by_drone / NT, 1 - drone_ratio_value, tr_avg / 3600, pf_few_value);
        }
    ...
    if (tr_file != NULL)
    {
        for (int i = 0; i < info_count; i++)
        {
            if (info_list[i].A_delivery_completed && info_list[i].A_delivery_completion_time > 0)
            {
                ...
                fprintf(tr_file, "shelter[%d] t = %f [hour] %.6f\n", info_list[i].shelter_id, info_list[i].A_delivery_completion_time / 3600, tr_hours); // 1行ずつTr値（時間単位）を出力 debug用
                total_tr_time += tr;
                completed_deliveries++;
            }
        }
