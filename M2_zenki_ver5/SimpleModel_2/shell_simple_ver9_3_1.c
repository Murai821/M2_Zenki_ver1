 ...
 * 　　　　E(TL)が物資の運搬遅延時間、
 * 11/17: 集積所複数について、物資運搬車両とドローンが「情報リスト: collect_inf_id[MAX_INFO]」を保持し、各集積所で情報を共有するように変更
...
// #define T_STOP_DIS (30 * 60)  // 各集積所での停止時間 (s) | 30分=1800秒/NDI
#define DETECTION_RADIUS 10.0 // ドローンの検出半径 (m) | 避難所近傍での情報検出・協調運搬判定用

// === シミュレーション設定 ===
#define NS 10        // 避難所の数（集積所除く）
#define NDI 6        // 集積所の数(NV：物資運搬車両の台数と同じにする)
