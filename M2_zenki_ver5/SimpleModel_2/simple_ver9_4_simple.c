 ...
 *       -> facilities[NS]をfacilities[NS+NDI]に変更（避難所集積所混合、参照時に-1する処理を除去し、current_stop_idxと完全対応）)
 ...
 * 　　　　E(TL)が物資の運搬遅延時間、
 * 11/17: 集積所複数について、物資運搬車両とドローンが情報リストを保持し、各集積所で情報を共有するように変更
 *        SupplyShelter構造体名 -> Facilityに変更, shelter_supplies[NS + NDI] -> facilities[NS + NDI]に変更
 * 11/18:generate_extra_supply_demand()修正, ドローンの物資運搬率のファイル出力追加
 * 　　　ドローンは巡回中に集積所を見つけるとリストを共有（複数集積所における集積所間のリスト交換）
 * 11/20:物資運搬車両が避難所の情報を検知するまでの遅延時間 Tc_Ave のコンソール表示（ドローンが先に発見し、集積所に共有した後、それを車両が集積所で初めて知る場合も含む）
 * 11/25日の報告会以降の検証プログラム
 * 11/26: ドローンが集積所へ情報を共有しに行くとき、最寄りの集積所へ行くのではなく、物資運搬車両が次に向かう集積所へ情報を共有しにいくように変更
...
#define MIN_EXTRA_DEMAND 0.0   // 余剰物資A需要量の最小値 (kg)
#define MAX_EXTRA_DEMAND 30.0  // 余剰物資A需要量の最大値 (kg)

// === 物資運搬車両システム ===
#define TOTAL_SUPPLY_WEIGHT NS * 1000.0 // 物資運搬車両の総積載量 (kg)
#define SUPPLY_G_PER_SHELTER 1000       // 1避難所あたりの物資G量 (kg)
#define EXTRA_SUPPLY_A 99999.0          // 余剰分の物資A量 (kg): 物資運搬車両は余剰物資Aを無制限に積載している想定
...
#define DELIVERY_METHOD_MULTI_DRONE 2 // 手法3：複数ドローンによる協調運搬(あるドローンが避難所で余剰物資Aを運搬中のとき、他のドローンが来たらそのドローンも協調して運搬する)
...
typedef struct
{
    ...
    double collection_time_later;      // ドローンがあとからやってきて情報を回収した時刻（秒）| -1の場合は未回収状態
    ...
    double extra_supply_delivered;     // 配送済み余剰物資A量 (kg)
    double B_extra_supply_delivered;   // 配送済み余剰物資B量 (kg)
    int delivery_completed;            // 余剰物資A配送完了フラグ（0:未完了, 1:完了）
    int B_delivery_completed;          // 余剰物資B配送完了フラグ（0:未完了, 1:完了）
    double delivery_completion_time;   // 余剰物資A配送完了時刻（秒）| -1の場合は未完了
    double B_delivery_completion_time; // 余剰物資Bの配送完了時刻（秒）| -1の場合は未完了
...
typedef struct
{
    double supply_g;               // 物資G(避難所に定期的に運搬する物資)の在庫量 (kg)
...
{
    double remaining_supply_g;               // 積載中の物資G残量 (kg)
...
typedef struct
{
    ...
    int required_trips;             // 必要な往復回数（A需要に基づく）
 ...
 *
 * ドローンの現在位置からDETECTION_RADIUS内の距離に集積所がある場合、
 * その集積所のdis_idx[NDI]を(集積所のID)返す。
 *
 * @param drone ドローン情報
 * @param stop_coords 拠点座標配列
 * @param dis_idx 集積所のインデックス配列
 * @return 検出した集積所の番号（検出なしの場合は-1）
 */
int check_drone_depot_detection(DroneInfo *drone, double stop_coords[][2], int dis_idx[NDI])
{
    // 各集積所との距離をチェック
...
int find_vehicle_next_depot(SupplyVehicle supply_vehicle[], int current_stop_idx[], int next_stop_idx[], DroneInfo drones[], int i, double stop_coords[][2], int dis_idx[NDI], int ndi_count, double vehicle_x[], double vehicle_y[])
{
    ...
    if (next_stop_idx[0] == supply_vehicle[0].next_depot_id)
    {
        ...
        double current_angle = atan2(vehicle_y[0], vehicle_x[0]);

        // 次の集積所の角度を計算
        double next_angle = PI / 2.0 - ve_next_depot_id * (2.0 * PI / (NS + NDI));

        // 角度差を計算（時計回り方向での最短角度）
        double angle_diff = current_angle - next_angle;
        ...
        // 円弧距離を計算（半径R × 角度）
        double arc_distance = R * angle_diff;
...
void update_drone_state(DroneInfo *drone, DroneInfo drones[], int drone_count, double stop_coords[][2], double elapsed_time, double time_step, Info *info_list, int info_count, Facility *facilities, double *total_extra_supply_by_drone, int *drone_delivery_count, int drone_index, int dis_idx[NDI], SupplyVehicle supply_vehicle[], int current_stop_idx[], int next_stop_idx[], double vehicle_x[], double vehicle_y[])
{
        ...
        if (elapsed_time >= drone->state_start_time + T_DRONE_STOP)
        {
            ...
            if (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE)
            {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    if (info_list[i].shelter_id == drone->target_shelter && !info_list[i].delivery_completed)
                    {
                        ...
                        remaining_demand = info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered - other_drone_transport;
                        break;
                    }
                }
            }
            else // 手法1,2
            {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    if (info_list[i].shelter_id == drone->target_shelter && !info_list[i].delivery_completed)
                    {
                        remaining_demand = info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered;
                        break;
                    }
                }
            }
            ...
            printf("ドローン: 集積所出発（避難所%dへ、余剰物資A %.0fkg積載）\n",
    ...
    case DRONE_AT_SHELTER:
        ...
        if (elapsed_time >= drone->state_start_time + T_DRONE_STOP)
        {
            ...
            for (int i = 0; i < info_count; i++)
            {
                if (info_list[i].shelter_id == drone->target_shelter && !info_list[i].delivery_completed && ((DELIVERY_METHOD != DELIVERY_METHOD_MULTI_DRONE && drone->delivery_info_index == i) || (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE)) && drone->delivery_info_index == i) // delivery_completedフラグが立っていない場合(手法1,2のみ情報のインデックス参照)
                {
                    drone->supply_scheduled_amount = 0; // 配送予定物資量リセット

                    // 避難所の余剰物資A在庫を増加
                    // facilitiesの配列インデックスは,current_stop_idxと同様なインデックス
                    ...
                    printf("ドローン: 避難所%dに余剰物資A %.0fkg配送（累計 %.0f/%.0fkg）\n",
                           drone->target_shelter, drone->carrying_extra_supply,
                           info_list[i].extra_supply_delivered, info_list[i].A_extra_supply_demand);
                    ...
                    *total_extra_supply_by_drone += drone->carrying_extra_supply;
                    ...
                    if (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE)
                    {
                        ...
                        if (info_list[i].extra_supply_delivered + total_other_drone_transport >= info_list[i].A_extra_supply_demand) // 避難所に届けられた物資量と他のドローンの運搬中の物資量が需要量を超えた場合
                        {
                            if (total_other_drone_transport > 0) //
                            {
                                printf("ドローン: 避難所%dの余剰物資A配送完了（他のドローンは継続中）他のドローン積載量:%f, 避難所%d の Req:%f \n", drone->target_shelter, total_other_drone_transport, drone->target_shelter, info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered);
                            }
                            else // 自分が最後にとどけた場合
                            {
                                info_list[i].delivery_completed = 1;
                                info_list[i].delivery_completion_time = elapsed_time;
                                printf("ドローン: 避難所%dの余剰物資A配送完了（全ドローン終了）, 避難所%d の Req:%f \n", drone->target_shelter, drone->target_shelter, info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered);
                            }
                        ...
                        else
                        {
                            ...
                            double remaining = info_list[i].A_extra_supply_demand - (info_list[i].extra_supply_delivered + total_other_drone_transport);
                    ...
                    else // 手法1と2
                    {
                        ...
                        if (info_list[i].extra_supply_delivered >= info_list[i].A_extra_supply_demand)
                        {
                            info_list[i].delivery_completed = 1;
                            info_list[i].delivery_completion_time = elapsed_time;
                            printf("ドローン: 避難所%dの余剰物資A配送完了, 避難所%d の Req:%f \n", drone->target_shelter, drone->target_shelter, info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered);
                            drone->current_trip = drone->required_trips + 999; // 往復完了フラグを設定（手法2によって,車両が避難所の余剰物資を運搬したことによるドローンの往復回数短縮を考慮して（車両によってドローンの往復必要回数がへる場合があるため））
                        }
                    }
                    // break;
...
/**
 * @brief ドローンによる指定避難所への余剰物資A運搬量を計算する関数
 *
            ...
            for (int j = 0; j < drone_count; j++)
            {
                ...
                if (drones[j].active && drones[j].target_shelter == shelter_id && drones[j].delivery_info_index == i)
                {
                    ...
                    case DRONE_AT_DEPOT:
                        ...
                        // 次の往復で運搬予定の物資量を計算
                        {
                            double remaining_demand = info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered;
...
int should_drone_join_transport(DroneInfo *drone, DroneInfo drones[], int drone_count, int shelter_id, Info *info_list, int info_count)
{
    ...
    {
        // 指定避難所IDかつ未配送完了
        if (info_list[i].shelter_id == shelter_id && info_list[i].delivery_completed == 0)
        {
                ...
                if (drones[j].active && drones[j].target_shelter == shelter_id && &drones[j] != drone && drones[j].delivery_info_index == i)
                {
                    ...
                    case DRONE_AT_DEPOT: // 集積所で物資積込中
                        // 次回往復で運搬予定の量を推定
                        {
                            ...
                            double remaining_demand = info_list[i].A_extra_supply_demand - (info_list[i].extra_supply_delivered + total_amount);
...
/**
 * @brief 全ドローンによる指定避難所への余剰物資A運搬量を計算する関数（手法3用）
 *
...
void plot_frame(FILE *pipe, double stop_coords[][2], double vehicle_x[], double vehicle_y[], DroneInfo drones[], double elapsed_time, Info *info_list, int info_count, Facility facilities[], SupplyVehicle supply_vehicle[], int dis_idx[NDI])
{
    ...
    // === 物資量可視化処理 ===
    // 各避難所の近くに物資量を数値で表示
    // 物資G（青色テキスト）と物資A（赤色テキスト）の在庫量を数値で表現
    ...
    for (int i = 1; i <= NS + NDI - 1; i++) // 避難所のみ（集積所は除く）
    {
        ...
        for (int j = 0; j < info_count; j++)
        {
            ...
            {
                current_demand += (info_list[j].A_extra_supply_demand - info_list[j].extra_supply_delivered);
            }
        }
        ...
        fprintf(pipe, "set label 'A:%.0f' at %.1f,%.1f tc rgb '#0000FF' font ',11'\n",
                facilities[i].supply_g, display_x_a, display_y_a);
 ...
 *
 * 【シミュレーション概要】
 * - 物資運搬車両が円形経路を巡回し、避難所に物資Gを配送
 * - ドローンが情報収集と余剰物資A配送を担当
 * - 情報発生タイミングはポアソン過程に従う
 *
 * 【実行フロー】
...
int main(void)
{
    ...
    for (int i = 0; i < ND; i++)
    {
        ...
        drones[i].carrying_extra_supply = 0.0;         // 積載中の余剰物資A量
    ...
    {
        facilities[i].supply_g = 0.0;       // 物資G在庫: 0kg
    ...
    // 現在は車両配列の[0]のみを使用
    supply_vehicle[0].remaining_supply_g = TOTAL_SUPPLY_WEIGHT;  // 物資G: 10000kg
    supply_vehicle[0].remaining_extra_supply_b = EXTRA_SUPPLY_A; // 余剰物資A: 1000kg
    ...
    supply_vehicle[0].next_depot_id = dis_idx[1]; // 次の集積所ID未設定
    supply_vehicle[0].next_depot_idx = 1;
    supply_vehicle[0].last_depot_arrival_time = 0.0; // 最後の集積所到着時刻初期化

    // == 物資運搬車両の周回時間（通常物資の運搬間隔）E(TI)計算 ==
    double TI_total_time = 0.0; // 1周にかかる総時間（秒）
    double TI_count = 0.0;      // 1周あたりの移動区間数
    ...
    while (lap_count < NT)
    {
        ...
        while (elapsed_time >= next_info_time && info_count < MAX_INFO)
        {
            ...
            info_list[info_count].collection_time_later = -1;                             // 後にドローンが回収した時刻: 未回収
            ...
            info_list[info_count].B_extra_supply_delivered = 0.0;                         // B配送済み量: 初期0kg
            info_list[info_count].delivery_completed = 0;                                 // 配送完了フラグ: 未完了
            info_list[info_count].B_delivery_completed = 0;                               // 余剰物資B配送完了フラグ: 未完了
            info_list[info_count].delivery_completion_time = -1;                          // 配送完了時刻: 未完了
            info_list[info_count].B_delivery_completion_time = -1;                        // 余剰物資Bの配送完了時刻: 未完了
            info_list[info_count].fast_detection_flag = 0;                                // 迅速発見フラグ: 未設定

            // 情報発生をログ出力（デバッグ・進捗確認用）
            printf("****情報発生: 時刻 %.1f秒 (%.1f分) : 避難所 %d (余剰物資A需要: %.0fkg)\n",
                   next_info_time, next_info_time / 60.0, info_list[info_count].shelter_id,
                   info_list[info_count].A_extra_supply_demand);
        ...
        if (!is_first_departure)
        {
            ...
            if (is_depot) // current_stop_idx[0] == 0など集積所インデックスと一致したとき
            {
                ...
                if (current_stop_idx[0] == 0)
                {
                    ...
                    TI_total_time += (elapsed_time - supply_vehicle[0].last_depot_arrival_time);
                    supply_vehicle[0].last_depot_arrival_time = elapsed_time; // 最後の集積所到着時刻を更新
                    TI_count += 1.0;
                }
                // == 物資運搬車両の次の集積所を設定 ==
                supply_vehicle[0].next_depot_idx = next_d;
                supply_vehicle[0].next_depot_id = dis_idx[next_d];
                ...
                for (int i = 0; i < info_count; i++)
                {
                    ...
                    if (supply_vehicle[0].collect_info_id[i] == 1 && facilities[current_stop_idx[0]].collect_info_id[i] == 0)
                    {
                        facilities[current_stop_idx[0]].collect_info_id[i] = 1; // 集積所での情報収集フラグを設定
                    }
                    // 集積所 -> 運搬車両
                    else if (supply_vehicle[0].collect_info_id[i] == 0 && facilities[current_stop_idx[0]].collect_info_id[i] == 1)
                    {
                ...
                {
                    supply_vehicle[0].remaining_supply_g = TOTAL_SUPPLY_WEIGHT;  // 物資G
                    supply_vehicle[0].remaining_extra_supply_b = EXTRA_SUPPLY_A; // 余剰物資A
                    supply_vehicle[0].is_loaded = 1;                             // 積載状態フラグを設定
                    printf("  物資補充: A=%.0fkg, 余剰B=%.0fkg を積載\n",
                           supply_vehicle[0].remaining_supply_g,
            ...
            else /***************** 避難所に停止する場合（current_stop_idx[] が０でないとき）******************************************/
            {
                ...
                // === 【避難所での物資配送処理】 ===
                // 車両が通常物資Gを積載中で、残量がある場合のみ配送実行
                if (supply_vehicle[0].is_loaded && supply_vehicle[0].remaining_supply_g > 0)
                {
                    // === 定量配送の実行 ===
                    // 各避難所に対して一定量（1000kg分）の物資Gを配送
                    facilities[current_stop_idx[0]].supply_g += SUPPLY_G_PER_SHELTER; // 避難所在庫増加

                    // 車両積載量から配送分を減算
                    supply_vehicle[0].remaining_supply_g -= SUPPLY_G_PER_SHELTER;

                    // 配送実行のログ出力
                    printf("  物資配送: A=%.0fkgを配送\n", SUPPLY_G_PER_SHELTER);
                    printf("  避難所%d在庫: A=%.0fkg\n",
                           current_stop_idx[0], facilities[current_stop_idx[0]].supply_g);

                    // === 車両積載状態の判定 ===
                    // 通常物資G空になった場合、積載状態を解除
                    // 余剰物資は別途管理されるため、残量があっても積載状態は解除される
                    if (supply_vehicle[0].remaining_supply_g <= 0)
                    {
                ...
                for (int i = 0; i < info_count; i++)
                {
                    ...
                    if (!info_list[i].collected && info_list[i].shelter_id == current_stop_idx[0] && supply_vehicle[0].remaining_extra_supply_b > 0)
                    {
                        // === 情報回収の実行 ===
                        ...
                        // === 【車両による余剰物資A配送処理】 ===
                        // 情報回収と同時に、その情報に関連する余剰物資A需要に対応
                        // 車両に余剰物資の残量があり、配送が未完了の場合のみ実行（手法1と手法2においてどちらも実行される）
                        if (supply_vehicle[0].remaining_extra_supply_b > 0 && !info_list[i].delivery_completed && supply_vehicle[0].demand_supply_loaded_flag[i] == 1)
                        {
                            ...
                            delivery_amount = (info_list[i].A_extra_supply_demand > supply_vehicle[0].remaining_extra_supply_b) ? supply_vehicle[0].remaining_extra_supply_b : info_list[i].A_extra_supply_demand;

                            // === 配送の実行 ===
                            if (delivery_amount > 0)
                            {
                                facilities[current_stop_idx[0]].extra_supply_a += delivery_amount; // 避難所在庫増加
                                ...
                                if (info_list[i].extra_supply_delivered >= info_list[i].A_extra_supply_demand)
                                {
                                    ...
                                    printf("  余剰物資A配送: %.0fkg配送（需要完了）\n", delivery_amount);
                                }
                                else
                                {
                                    printf("  余剰物資A配送: %.0fkg配送（残り需要 %.0fkg）\n",
                                           delivery_amount, info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered);
                                }
                            }
                            else
                            {
                                printf("  余剰物資A配送: ドローンが運搬中のため車両配送スキップ\n");
                            }
                        }
                    }
                }

                // == すでにドローンによって情報が回収されているがあとから運搬車両が到着した回収処理
                for (int i = 0; i < info_count; i++)
                {
                    // 対象条件: ドローンによって既に情報が取得されている、情報を取得
                    if (info_list[i].shelter_id == current_stop_idx[0] && info_list[i].collected_by == COLLECTED_BY_DRONE)
                    {
                        supply_vehicle[0].collect_info_id[i] = 1; // 車両の回収情報IDリストに登録
                    }
                }
                ...
                for (int i = 0; i < info_count; i++)
                {
                    if (ND >= 0) // ドローンを導入しない場合は余剰物資Aも物資運搬車両が全て配送する、また、ドローンを導入する場合もドローンで運搬しきれない物資Aも、集積所で所望の物資を積載した上で運搬
                    {
                        ...
                        if (info_list[i].shelter_id == current_stop_idx[0] && supply_vehicle[0].demand_supply_loaded_flag[i] == 1 && supply_vehicle[0].remaining_extra_supply_b > 0 && !info_list[i].delivery_completed)
                        {
                            ...
                            if (delivery_amount > 0)
                            {
                                ...
                                if (info_list[i].extra_supply_delivered >= info_list[i].A_extra_supply_demand) // 運搬完了したら
                                {
                                    info_list[i].delivery_completed = 1;
                                    info_list[i].delivery_completion_time = elapsed_time;
                                    printf("  余剰物資A配送: %.0fkg配送（需要完了）\n", delivery_amount);
                ...
                // === 【避難所での余剰物資B配送処理】一度集積所で情報を回収し需要に合ったものを積載してきてから（自分で回収した情報で需要に合ったものを積載も含まれる） ===
                for (int i = 0; i < info_count; i++)
                {
                    // 対象条件: 現在の避難所で発生した情報, 要求物資積載フラグが立っている, かつ配送未完了,余剰物資あり
                    if (info_list[i].shelter_id == current_stop_idx[0] && supply_vehicle[0].demand_supply_loaded_flag[i] == 1 && supply_vehicle[0].remaining_extra_supply_b > 0 && !info_list[i].B_delivery_completed)
                    {
                        double delivery_amount = 0.0;

                        delivery_amount = (info_list[i].B_extra_supply_demand > supply_vehicle[0].remaining_extra_supply_b) ? supply_vehicle[0].remaining_extra_supply_b : info_list[i].B_extra_supply_demand;

                        // === 配送の実行 ===
                        if (delivery_amount > 0)
                        {
                            facilities[current_stop_idx[0]].extra_supply_b += delivery_amount; // 避難所在庫増加
                            supply_vehicle[0].remaining_extra_supply_b -= delivery_amount;
                            ...
                            {
                                info_list[i].B_delivery_completed = 1;
                                info_list[i].B_delivery_completion_time = elapsed_time;
                                printf("  余剰物資B配送: %.0fkg配送（需要完了）\n", delivery_amount);
                            }
                            else
                            {
                                printf("  余剰物資B配送: %.0fkg配送（残り需要 %.0fkg）\n",
                                       delivery_amount, info_list[i].B_extra_supply_demand - info_list[i].B_extra_supply_delivered);
                            }
                        }
                    }
                }
                // 余剰物資Aに関する処理
                //  ====== 手法２：避難所の情報は回収済みだが、ドローンによる余剰物資A配送が未完了のときに車両が到着した場合(手法2:すべてドローンに任せるのではなく車両がのこりの物資を運搬する)
                // ドローンを導入するとき，余剰物資Aドローンの運搬に多くの時間がかかる場合，所望の物資を積載した物資運搬車両が避難所に到着するまでに，ドローンが物資を運搬し終わらない場合がある
                //
                for (int i = 0; i < info_count; i++)
                {
                    // 現在の避難所において
                    if (info_list[i].shelter_id == current_stop_idx[0])
                    {
                        ...
                        {
                            // === 【車両による余剰物資A配送処理】 ===
                            // 車両に余剰物資Aの残量があり、ドローンによる配送が未完了の場合のみ実行
                            if (supply_vehicle[0].remaining_extra_supply_b > 0 && !info_list[i].delivery_completed && info_list[i].A_extra_supply_demand > info_list[i].extra_supply_delivered)
                            {
                                ...
                                if (ND != 0 && (DELIVERY_METHOD == DELIVERY_METHOD_COORDINATE || DELIVERY_COORDINATATE_FLAG == 1)) // ドローンが存在し、手法２を適用するとき
                                {
                                    ...
                                    double remaining_demand = info_list[i].A_extra_supply_demand - info_list[i].extra_supply_delivered - drone_transport_amount;
                                    ...
                                    printf("  配送量計算: 避難所%d, 需要%.0fkg - 既配送%.0fkg - ドローン運搬%.0fkg = 残り需要%.0fkg → 車両配送%.0fkg\n",
                                           info_list[i].shelter_id, info_list[i].A_extra_supply_demand, info_list[i].extra_supply_delivered, drone_transport_amount, remaining_demand, delivery_amount);
                            ...
                            if (detected_shelter > 0) // 情報を検出した場合
                            {
                                ...
                                for (int j = 0; j < info_count; j++)
                                {
                                        ...
                                        info_list[j].generation_time <= elapsed_time)
                                    {
                                        ...
                                        printf("ドローン%d: 避難所%dで情報検出・回収（余剰物資A需要: %.0fkg, 往復回数: %d回）\n",
                                               i + 1, detected_shelter, info_list[j].A_extra_supply_demand, drones[i].required_trips);
                            ...
                            else // 情報を検出しなかった場合
                            {
                                if (DELIVERY_METHOD == DELIVERY_METHOD_MULTI_DRONE) // 手法3のとき
                                {
                                    ...
                                    if (cooperative_shelter > 0)
                                    {
                                        ...
                                        for (int j = 0; j < info_count; j++)
                                        {
                                            if (info_list[j].shelter_id == cooperative_shelter && !info_list[j].delivery_completed)
                                            {
                                                double remaining_demand = info_list[j].A_extra_supply_demand - info_list[j].extra_supply_delivered;
        ...
        // next_stop_idx[NV]; // 各車両の次の停止地点インデックス
        ...
        for (int v = 0; v < NV; v++)
        {
            ...
            if (v == 0)
            {
                printf("****************************************車両%d: 次の停止地点=%d\n", v + 1, current_stop_idx[v]);
            }
        }
    }
    ...
    for (int i = 0; i < info_count; i++)
    {
        ...
        if (info_list[i].collected_by_drone_later == 1 && info_list[i].collection_time_later >= 0)
        {
            printf("ドローンによる後続回収が発生しました\n");
            double tc_later = info_list[i].collection_time_later - info_list[i].generation_time;
            printf("tc_later: %.2f\n", tc_later / 3600);
    ...
    if (tv_collection_count > 0)
    {
        ...
        printf("Tc_TV（車両回収時間）平均: %.2f秒 (%.2f分)\n", tc_tv_avg, tc_tv_avg / 60.0);
    ...
    // -> ドローンが物資を運搬している時間のうち、ドローンが少量の物資を運搬している割合"
    ...
    if (numerical_data_file != NULL)
    {
        ...
        {
            fprintf(numerical_data_file, "%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n",
                    zero_value, vehicle_tc_avg / 3600, drone_tc_avg / 3600, all_tc_avg / 3600,
                    total_extra_supply_by_vehicle / NT, total_extra_supply_by_drone / NT, zero_value, tr_avg / 3600, zero_value);
        }
        else
        {
            fprintf(numerical_data_file, "%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n",
        ...
        for (int i = 0; i < info_count; i++)
        {
            if (info_list[i].B_delivery_completed && info_list[i].delivery_completion_time > 0)
            {
                double tl = info_list[i].B_delivery_completion_time - info_list[i].generation_time;
                total_tl_time += tl;
    ...
    if (tl_file != NULL)
    {
        for (int i = 0; i < info_count; i++)
        {
            if (info_list[i].B_delivery_completed && info_list[i].B_delivery_completion_time > 0)
            {
                double tl = info_list[i].B_delivery_completion_time - info_list[i].generation_time;
                double tl_hours = tl / 3600.0; // 秒から時間に変換
                // fprintf(tl_file, "%.6f\n", tl_hours); // 1行ずつTl値（時間単位）を出力
                fprintf(tl_file, "shelter[%d] t = %f [hour] %.6f\n", info_list[i].shelter_id, info_list[i].B_delivery_completion_time / 3600, tl_hours); // 1行ずつTl値（時間単位）を出力 debug用
                total_tl_time += tl;
                completed_tl_deliveries++;
            }
        }
    ...
    // ====== TLについて各避難所ごとにの平均値を計算してファイル出力 ========================

    // 既存のtl_shelter_avg.txtから値を読み込む
    ...
    FILE *existing_total_file = fopen("Results/tl_shelter_avg_total.txt", "r");
    if (existing_total_file != NULL)
    {
        ...
        printf("既存のtl_shelter_avg_total.txtから%d行の累積値とカウンタを読み込みました\n", line_count);
    }
    else
    {
        // total.txtがない場合は、通常のtl_shelter_avg.txtから読み込む
        FILE *existing_file = fopen("Results/tl_shelter_avg.txt", "r");
        if (existing_file != NULL)
        {
            ...
            printf("既存のtl_shelter_avg.txtから%d行の値を読み込みました（初回カウント）\n", line_count);
        }
        else
        {
    ...
    FILE *tl_shelter_file = fopen("Results/tl_shelter_avg.txt", "w");
    FILE *tl_shelter_total_file = fopen("Results/tl_shelter_avg_total.txt", "w");

    if (tl_shelter_file != NULL && tl_shelter_total_file != NULL)
    {
        // 避難所ごとのTL平均値を計算
        for (int shelter_id = 1; shelter_id <= NS + NDI - 1; shelter_id++)
        {
            double total_tl_shelter = 0.0;
            int count_tl_shelter = 0;

            for (int i = 0; i < info_count; i++)
            {
                if (info_list[i].shelter_id == shelter_id && info_list[i].B_delivery_completed && info_list[i].B_delivery_completion_time > 0)
                {
                    double tl = info_list[i].B_delivery_completion_time - info_list[i].generation_time;
                    total_tl_shelter += tl;
                    count_tl_shelter++;
            ...
            if (count_tl_shelter > 0)
            {
                double avg_tl_shelter = total_tl_shelter / count_tl_shelter;
                new_avg_value = avg_tl_shelter / 3600.0;
                // fprintf(tl_shelter_file, "shelter[%d]: Tl平均 = %.2f秒 (%.2f時間)\n", shelter_id, avg_tl_shelter, avg_tl_shelter / 3600.0);
                fprintf(tl_shelter_file, "%.2f\n", new_avg_value);
            }
            else
            {
                new_avg_value = 0.0;
            ...
            fprintf(tl_shelter_total_file, "%.2f %d\n", total_value, new_count);
        }

        fclose(tl_shelter_file);
        fclose(tl_shelter_total_file);
        printf("避難所ごとのTl平均値をResults/tl_shelter_avg.txtに出力しました\n");
        printf("避難所ごとのTl平均値の累積和とカウンタをResults/tl_shelter_avg_total.txtに出力しました\n");
    }
    else
    {
        if (tl_shelter_file == NULL)
        {
            printf("警告: Tl避難所別平均値ファイル(tl_shelter_avg.txt)の作成に失敗しました\n");
        }
        if (tl_shelter_total_file == NULL)
        {
            printf("警告: Tl避難所別平均値累積ファイル(tl_shelter_avg_total.txt)の作成に失敗しました\n");
        }
        if (tl_shelter_file != NULL)
            fclose(tl_shelter_file);
        if (tl_shelter_total_file != NULL)
    ...
    // == 提案手法による物資Lの配送が短縮された割合（確率）に関する統計 ========================================================
    int shortened_count = 0;
    int total_l_deliveries = 0;
    for (int i = 0; i < info_count; i++)
    {
        if (info_list[i].B_delivery_completed)
        {
            total_l_deliveries++;
    ...
    printf("\n=== 物資L配送短縮割合統計 ===\n");
    if (total_l_deliveries > 0)
    {
        double shortening_ratio = ((double)shortened_count / total_l_deliveries) * 100.0;
        printf("物資L配送短縮件数: %d件 / %d件 (%.2f%%)\n", shortened_count, total_l_deliveries, shortening_ratio);

        // === Rsh.txt ファイルの出力（物資L配送短縮割合） ===
        ...
        if (shortening_ratio_file != NULL)
        {
            ...
            printf("物資L配送短縮割合をResults/Rsh.txtに追記しました\n");
        }
        else
        {
    ...
    {
        printf("物資L配送完了件数が0のため、短縮割合は計算できません\n");

        // === Rsh.txt ファイルの出力（物資L配送短縮割合が計算できない場合） ===
        FILE *shortening_ratio_file = fopen("Results/Rsh.txt", "a");
        if (shortening_ratio_file != NULL)
        {
            fprintf(shortening_ratio_file, "0.000\n"); // 配送完了件数が0の場合は0を出力
            fclose(shortening_ratio_file);
            printf("物資L配送短縮割合をResults/Rsh.txtに追記しました（0値）\n");
        }
        else
        {
            printf("警告: Rsh.txtファイルの作成に失敗しました\n");
    ...
    // == TI (通常物資の平均運搬間隔) 統計 ===============================================================
    printf("\n=== TI (通常物資の平均運搬間隔) 統計 ===\n");
    double E_TI = TI_total_time / TI_count;
    printf("%.2f,%.2f\n", TI_total_time, TI_count); // debug用
    printf("E_TI: %.2f [hour]\n", E_TI / 3600.0);
