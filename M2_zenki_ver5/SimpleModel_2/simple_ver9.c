 ...
 *       ④手法5に手法2を適用できるように変更
 * 10/27: 集積所と避難所の円の中心が(0,0)であったところを, 集積所を(0,0)になるように修正。集積所の下に,避難所のみの円を独立で配置。避難所1と集積所間は直線
 *        （順回路をふやすことを踏まえた変更）
 * a
 */
#include <stdio.h>
...
// === 派生的な定数（自動計算） ===
#define TOTAL_STOPS (NS + NDI) + 1     // 集積所を含めた拠点の総数（12拠点）避難所1は2回到達するから
#define DRAW_INTERVAL 120.0            // 描画間隔（秒）| 2分ごとにGIFフレーム生成
const double shelter_distance = 500.0; // 集積所から避難所1までの距離
...
typedef struct // 物資運搬車両管理構造体
{
    ...
    int delivery_finished;           // 配送完了フラグ（0:未完了, 1:完了）
} SupplyVehicle;
...
void update_drone_state(DroneInfo *drone, DroneInfo drones[], int drone_count, double stop_coords[][2], double elapsed_time, double time_step, Info *info_list, int info_count, ShelterSupply *shelter_supplies, double *total_extra_supply_by_drone, int *drone_delivery_count)
{
    ...
    case DRONE_PATROL:
        // 通常巡回（設定可能な方向）
        {
            ...
            // === 座標更新 ===
            // 角度に基づいて円周上の座標を計算
            // 新座標系では集積所(0,0)を中心とした円周上を移動
            drone->x = R * cos(drone->angle);
            drone->y = R * sin(drone->angle) - R - shelter_distance;
        }
        break;
...
void plot_frame(FILE *pipe, double stop_coords[][2], double vehicle_x[], double vehicle_y[], DroneInfo drones[], double elapsed_time, Info *info_list, int info_count, ShelterSupply shelter_supplies[], SupplyVehicle supply_vehicle[])
{
    ...
    // === 描画コマンドの構築 ===
    // 基本レイヤー（円、集積所-避難所1直線、集積所、避難所、車両）を描画
    fprintf(pipe, "plot '-' with lines lc 'gray' lw 2 notitle, ");                              // 1. 円周経路
    fprintf(pipe, "'-' with lines lc 'gray' lw 2 notitle, ");                                   // 2. 集積所-避難所1直線
    ...
    fprintf(pipe, "'-' with points pointtype 5 pointsize 3 linecolor rgb '#FF00FF' notitle, "); // 6. 集積所強調
    fprintf(pipe, "'-' with points pt 7 ps 2.2 lc 'blue' notitle");                             // 7. 車両

    // ドローンレイヤーは台数が1台以上の場合のみ追加
    if (ND > 0)
    {
        fprintf(pipe, ", '-' with points pt 5 ps 1.8 lc 'green' notitle"); // 8. ドローン群
    }
    fprintf(pipe, "\n");

    // === データ送信開始 ===

    // 1. 経路円の座標データ送信（集積所を中心とした円、避難所2-10のみ）
    for (int i = 0; i <= 360; i += 5)
    {
        double angle = i * PI / 180.0;       // 度 → ラジアン変換
        double x = R * cos(angle);           // 集積所中心の円のX座標
        double y = R * sin(angle) - R - 500; // 集積所中心の円のY座標
        fprintf(pipe, "%.1f %.1f\n", x, y);
    }
    fprintf(pipe, "e\n"); // データ終了マーカー

    // 2. 集積所-避難所1間の直線データ送信
    fprintf(pipe, "%.1f %.1f\n", stop_coords[0][0], stop_coords[0][1]); // 集積所座標
    fprintf(pipe, "%.1f %.1f\n", stop_coords[1][0], stop_coords[1][1]); // 避難所1座標
    fprintf(pipe, "e\n");                                               // データ終了マーカー
    ...
    // 8. ドローン群の現在位置を送信（緑色表示）- ドローン台数>0の場合のみ
    if (ND > 0)
    {
...
int main(void)
{
    ...
    const double angle_increment = 2.0 * PI / NS; // 避難所2-10間の角度差（9個の避難所で円形）

    // 【新座標系】
    // - 集積所: (0, 0) に固定配置
    // - 避難所1: 集積所の真下 (0, -500) に配置（直線接続）
    // - 避難所2-10: 集積所を中心とした円周上に等間隔配置

    // 集積所の座標
    stop_coords[0][0] = 0.0; // 集積所X座標
    stop_coords[0][1] = 0.0; // 集積所Y座標

    // 避難所1の座標（集積所の真下、直線接続）
    stop_coords[1][0] = 0.0;               // 避難所1 X座標
    stop_coords[1][1] = -shelter_distance; // 避難所1 Y座標（集積所から真下に500m）

    // 避難所2-10の座標（集積所を中心とした円周上に等間隔配置）
    for (int i = 2; i <= NS; i++)
    {
        // 角度計算: 12時位置(π/2)から開始し、時計回りに等間隔配置
        double angle = PI / 2.0 - (i - 1) * angle_increment;       // 避難所2から順に配置
        stop_coords[i][0] = R * cos(angle);                        // 円周上のX座標
        stop_coords[i][1] = R * sin(angle) - R - shelter_distance; // 円周上のY座標
    }
    ...
    // === 各ドローンの初期化 ===
    // 全ドローンを集積所（集積所座標(0,0)）に配置し、順次出発スケジュールを設定
    for (int i = 0; i < ND; i++)
    {
        drones[i].angle = PI / 2.0;                          // 全て集積所（12時位置、π/2ラジアン）から開始（角度は維持）
        drones[i].x = stop_coords[0][0];                     // 初期X座標（集積所の座標）
        drones[i].y = stop_coords[0][1];                     // 初期Y座標（集積所の座標）
    ...
    supply_vehicle[0].delivery_finished = 0;                                     // 配送完了フラグ: 未完了
    ...
    while (lap_count < NT)
    {
        ...
        if (!is_first_departure)
        {
            ...
            if (current_stop_idx[0] == 0)
            {
                ...
                supply_vehicle[0].delivery_finished = 0; // 集積所到着で配送完了フラグをリセット
            }
            else /***************** 避難所に停止する場合（current_stop_idx[] が０でないとき）******************************************/
            {
                printf("周回: %d/%d | 避難所%dに到着。%d秒間停止します。\n", lap_count + 1, NT, current_stop_idx[0], T_STOP);

                if (current_stop_idx[0] == NS)
                {
                    supply_vehicle[0].delivery_finished = 1; // 最後の避難所到着で配送完了フラグを設定
                }
        ...
        for (int v = 0; v < NV; v++)
        {
            ...
            if (current_stop_idx[v] == NS) // すべての避難所を訪問したあとは、一度避難所１に戻る
            {
                next_stop_idx[v] = 1;
            }
            else if (current_stop_idx[v] == 1 && supply_vehicle[v].delivery_finished == 1) // 避難所１に到着したときに、物資が空の場合は集積所に戻る(2回目に避難所1に到着したとき)
            {
                next_stop_idx[v] = 0;
            }
            else
            {
                next_stop_idx[v] = current_stop_idx[v] + 1;
            }
        }
        ...
        for (int v = 0; v < NV; v++)
        {
            start_angle[v] = PI / 2.0 - (current_stop_idx[v] - 1) * angle_increment; // 出発地点の角度
            end_angle[v] = PI / 2.0 - (next_stop_idx[v] - 1) * angle_increment;      // 到着地点の角度
        }
        ...
        while (elapsed_time < move_end_time)
        {
            ...
            for (int v = 0; v < NV; v++)
            {
                ...
                // 新座標系における車両位置計算
                // 集積所↔避難所1間は直線移動、それ以外は角度ベース移動
                if ((current_stop_idx[v] == 0 && next_stop_idx[v] == 1) ||
                    (current_stop_idx[v] == 1 && next_stop_idx[v] == 0))
                {
                    // 集積所↔避難所1間の直線移動
                    double start_x = stop_coords[current_stop_idx[v]][0];
                    double start_y = stop_coords[current_stop_idx[v]][1];
                    double end_x = stop_coords[next_stop_idx[v]][0];
                    double end_y = stop_coords[next_stop_idx[v]][1];

                    current_x[v] = start_x + (end_x - start_x) * progress;
                    current_y[v] = start_y + (end_y - start_y) * progress;
                }
                else
                {
                    // 避難所2-10間の角度ベース移動（従来の円周移動）
                    double original_x = R * cos(current_angle[v]);
                    double original_y = R * sin(current_angle[v]);
                    current_x[v] = original_x - R * cos(PI / 2);
                    current_y[v] = original_y - R * sin(PI / 2) - shelter_distance;
                }
            }
...
void save_simulation_model_png(double stop_coords[][2])
{
    ...
    // 1. 円形道路（集積所を中心とした円、避難所2-10用）
    fprintf(pipe, "%.1f*cos(t), %.1f*sin(t) - %.1f - %.1f with lines linewidth 3 linecolor rgb '#808080' notitle, ",
            R, R, R, shelter_distance);

    // 2. 集積所-避難所1間の直線道路
    fprintf(pipe, "'-' with lines linewidth 3 linecolor rgb '#808080' notitle, ");
    ...
    // 集積所-避難所1間の直線データ
    fprintf(pipe, "%.1f %.1f\n", stop_coords[0][0], stop_coords[0][1]); // 集積所
    fprintf(pipe, "%.1f %.1f\n", stop_coords[1][0], stop_coords[1][1]); // 避難所1
    fprintf(pipe, "e\n");
