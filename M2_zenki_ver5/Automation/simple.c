 ...
 * - gnuplot (グラフ描画ツール)
 *
 * ver1: ANIMATION_FRAMESを10に設定し、停止時と移動時のフレームを固定にしたバージョン
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#define T_STOP (5 * 60)      // 各拠点での停止時間 (s) | 10分
...
#define ANIMATION_FRAMES 10  // アニメーションの滑らかさ (1区間あたりの描画フレーム数) - 処理軽量化のため削減
...
FILE *init_gnuplot()
{
    ...
    fprintf(pipe, "set terminal gif animate delay 10 size 600,600 crop\n");
...
int main(void)
{
    ...
    // gnuplotパイプの初期化
    FILE *gnuplot_pipe = init_gnuplot();
    ...
    const long frame_delay_us = (long)((travel_time_per_segment / ANIMATION_FRAMES) * 1000000); // アニメーション用遅延時間
    ...
    while (lap_count < NT)
    {
        ...
        if (!is_first_departure)
        {
            ...
            // 停止時間中の描画処理
            // 移動フレーム数と同じフレーム数で停止時間を表現（時間の流れを統一）
            const int stop_frames = ANIMATION_FRAMES;                // 移動フレーム数と同じに統一
            const double time_per_stop_frame = T_STOP / stop_frames; // 1フレームあたりの時間

            for (int i = 0; i < stop_frames; ++i)
            {
                plot_frame(gnuplot_pipe, stop_coords, current_x, current_y, elapsed_time, info_list, info_count);
                // 停止時間中の待機時間を移動時間と同じに統一
                usleep(40000); // 0.04秒の遅延（移動時間と同じ）

                // 停止時間を徐々に経過時間に追加（時間の流れを移動時間と統一）
        ...
        double angle_change_per_frame = -angle_increment / ANIMATION_FRAMES; // フレーム毎の角度変化
        ...
        for (int frame = 1; frame <= ANIMATION_FRAMES; frame++)
        {
            ...
            // 移動中の車両位置を描画
            plot_frame(gnuplot_pipe, stop_coords, current_x, current_y, elapsed_time, info_list, info_count);
            usleep(40000); // 0.04秒の遅延（処理軽量化のため増加）

            // 移動時間を徐々に経過時間に追加
            elapsed_time += travel_time_per_segment / ANIMATION_FRAMES;
        }
